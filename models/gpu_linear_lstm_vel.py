...
class TrajectoryPredictor:
  ...
  def __init__(self, input_dim, middle_dim, output_dim, batch_size):
    ...
    self.linear = torch.nn.Linear(input_dim, middle_dim).cuda()
    self.lstm = torch.nn.LSTM(middle_dim, output_dim).cuda()
      ...
      for data in train_loader:
        ...
        for i in range(first_part_len):
          ...
          inp = torch.cat((traj_first_part[:,i,:], v_x.contiguous().view(actual_batch_size, 1), \
                          v_y.contiguous().view(actual_batch_size, 1)), dim=1).contiguous().view(actual_batch_size, self.input_dim)
          tmp = self.linear(inp)
        ...
        for i in range(second_part_len-1):
          ...
          tmp = self.linear(inp)
          pred, self.state = self.lstm(tmp.contiguous().view(1, actual_batch_size, self.middle_dim), self.state)
  ...
  def test(self, test_loader):
    ...
    for data in test_loader:
      ...
      for i in range(second_part_len-1):
        ...
        prev_out = out
        inp = torch.cat((out, v_x.contiguous().view(1,actual_batch_size,1), v_y.contiguous().view(1,actual_batch_size,1)), dim=2).contiguous().view(actual_batch_size, self.input_dim)
...
if __name__ == '__main__':
  train_trajectories = []
  for filename in os.listdir('/xy_data/train_data'):
    trajectories = np.load(filename)
    train_trajectories.append(trajectories)

  train_trajectories = np.stack(train_trajectories)
  train_data = train_trajectories[:, :10, :]
  train_target = train_trajectories[:, 10:, :]

  dev_trajectories = []
  for filename in os.listdir('/xy_data/dev_data'):
    trajectories = np.load(filename)
    dev_trajectories.append(trajectories)
  
  dev_trajectories = np.stack(dev_trajectories)
