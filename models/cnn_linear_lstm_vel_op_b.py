...
class TrajectoryPredictor:
  ...
  def __init__(self, input_dim, middle_dim, output_dim, batch_size):
    ...
    self.cnn = torch.nn.Sequential(*list(alexnet.features.children())[:-1])
  ...
  def train(self, train_loader, num_epochs, N):
    ...
    for epoch in range(num_epochs):   
      ...
      for data in train_loader:
        ...
        for i in range(first_part_len):
          ...
          # example for reshaping input into input for CNN
          cnn_input = traj_first_part[:,i,2:].contiguous().view(actual_batch_size, 3, N, N)
